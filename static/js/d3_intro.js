;(function () {
  ...
  var visualisation = {
    domtree: {
      ...
      data: [],
      loadData: function () {
        var walkDOM = function (node, root, func) {
      ...
      visualise: function () {
        ...
        var cluster = d3.layout.tree().size([height - 100, width - 160])
        ...
        node
          ...
          .attr('class', function (d) {
            return d.children ? 'children' : 'noChildren'
          })
          .attr('r', 4.5)
    ...
    basic: {
      ...
      visualise: function () {
        ...
        var circles = svg.selectAll('circle').data(this.data).enter().append('circle')
    ...
    interaction: {
      ...
      visualise: function () {
        ...
        var xScale = d3.scale.ordinal().domain(d3.keys(this.data)).rangeRoundBands([0, width], 0.1)

        var yScale = d3.scale
          .linear()
          .domain([
            0,
            d3.max(d3.entries(this.data), function (d) {
              return d.value / 100
            }),
          ])
          .range([height, 0])
